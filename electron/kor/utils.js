const { CiferKor } = require("../db/schema")
...
async function validateDocument(obj){
    ...
    for(let unitObj of obj){
        ...
        let objFromMongoDB = await CiferKor.findOne({chinaRegNo: chinaRegNo})
        ...
        if(objFromMongoDB){
            ...
            if(toBeUpdated.judge) {
                await CiferKor.findOneAndUpdate({chinaRegNo: chinaRegNo}, unitObj)
    ...
    let MongoDBdocsArr = await CiferKor.find({}, {_id:0, __v:0})
    if(MongoDBdocsArr.length > 0){
        for(let dbDoc of MongoDBdocsArr){
            ...
            if(!notDeleted) {
                ...
                await CiferKor.findOneAndDelete({chinaRegNo: dbDoc.chinaRegNo})
            }
        }
    }
